...
def main():
    ...
    try:
        ...
        logger.info(
            "If it exists, it will be used and upgraded if needed (no data loss)."
        )
        logger.info("If it does not exist, a new database will be created.")
        logger.info(
            "Group filter will be determined from console or JSON file (if provided)."
        )
        print("-" * 80)
        ...
        if config.continuous_mode:
                ...
                if status == "success":
                    ...
                    if today_data:
                        ...
                        for record in today_data:
                            ...
                            try:
                                logger.info(
                                    f"  {name}: {status_text} {time_info} [{state}]"
                                )
                            except UnicodeEncodeError:
                                logger.info(
                                    f"  [Ukrainian group]: [Ukrainian status] {time_info} [{state}]"
                                )
                    ...
                    logger.info("\nICS Files Usage:")
                    logger.info(
                        "  - Individual .ics files: Import each file separately"
                    )
                    logger.info("  - Combined .ics file: Import all events at once")
                    logger.info(
                        "  - Cancellation .ics file: Import to remove old events"
                    )
                    logger.info(
                        "  - Manual deletion .txt: Backup option for manual removal"
                    )
            ...
            logger.info(
                "  python main.py --continuous --interval 300   # Every 5 minutes"
            )
            logger.info(
                "  python main.py --continuous --interval 900   # Every 15 minutes"
            )
            ...
            logger.info(
                "If neither is provided, all groups from the website will be processed."
            )

            logger.info("=" * 80)
            logger.info(
                "MONITORING SETUP COMPLETE - ICS CALENDAR SUPPORT WITH UKRAINE TIMEZONE"
            )
            logger.info("Check calendar_events/ folder for .ics files to import")
            logger.info(
                "All calendar events use Ukraine timezone (Europe/Kiev) with DST support"
            )
            logger.info("=" * 80)
        ...
        logger.info(
            f"Old records cleanup completed (older than {config.cleanup_days} days)"
        )
        print("-" * 80)
        print("Exit.")
